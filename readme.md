...
ไม่ต้องติดตั้ง `llama-cpp-python` — script เรียกผ่าน Ollama API โดยตรง
...
```bash
# แสดงผลใน terminal
python compare_results.py macbook_air_m3.json mac_mini_m4.json dell_pro_ryzen.json

# แสดงผลใน terminal + บันทึก Markdown
python compare_results.py macbook_air_m3.json mac_mini_m4.json dell_pro_ryzen.json --md results.md
```
...
กรณีที่ Ollama รันบนเครื่องเดียวกัน ไม่ต้องตั้งค่าอะไรเพิ่มเติม

---

## การทดสอบทั้งหมด (น้ำหนักรวม 100%)

| # | การทดสอบ | เครื่องมือ | metric | higher is better | น้ำหนัก |
|---|----------|-----------|--------|:---:|---------|
...
| T6 | CPU Compute (NumPy matmul 4K×4K) | NumPy | GFLOPS | ✓ | 10% |

T1 ใช้โมเดลที่ pull ไว้ใน Ollama แล้ว — ไม่มีการดาวน์โหลดอัตโนมัติ  
T3 และ T4 วัดเป็นวินาที ยิ่งน้อยยิ่งดี (higher is better = ✗)
...
```
score_i = (result_i / best) × 100   # higher-is-better: tokens/sec, images/sec, GB/s, GFLOPS
score_i = (best / result_i) × 100   # lower-is-better:  seconds (T3, T4)
...
แต่ละ platform ใช้ hardware accelerator ของตัวเองได้เต็มที่ (Metal / CoreML / DirectML)
...
```
benchmark/
├── run_benchmark.py      ← รัน benchmark บนเครื่องนี้ → ได้ JSON
├── compare_results.py    ← เปรียบเทียบ JSON ข้ามเครื่อง → terminal + Markdown
